 ...
 *        протокола из настроек (ARCH §8, Фаза 3.3), 3 зарегистрированных
 *        драйвера (НКУ-CAN, демо, УИМ-6100).
 */
...
{
    TEST_ASSERT_EQUAL_UINT8(3U, sul_registry_count()); /* НКУ-CAN + демо + УИМ */
}

/* id каждого драйвера УНИКАЛЕН. Ловит реальный класс ошибки: копипаст записи
 * реестра с чужим .id делает протокол НЕДОСТИЖИМЫМ (find() отдаёт первое
 * совпадение), причём молча — set_active() трактует "не найден" как мусор в
 * настройках и просто игнорирует. */
static void test_driver_ids_are_unique(void)
{
    const uint8_t COUNT = sul_registry_count();

    for (uint8_t id = 0U; id < COUNT; id++)
    {
        const sul_driver_t *p_drv = sul_registry_find(id);
        TEST_ASSERT_NOT_NULL_MESSAGE(p_drv, "id из диапазона 0..count-1 должен находиться");
        TEST_ASSERT_EQUAL_UINT8(id, p_drv->id);
    }
}

static void test_find_returns_uim(void)
{
    const sul_driver_t *p_drv = sul_registry_find(SUL_PROTOCOL_UIM);
    TEST_ASSERT_NOT_NULL(p_drv);
    TEST_ASSERT_EQUAL_STRING("УИМ-6100", p_drv->p_name);
}
...
static void test_take_pending_write_wired_only_for_nku_can(void)
{
    ...
    TEST_ASSERT_NULL(sul_registry_find(SUL_PROTOCOL_UIM)->take_pending_write);
}

/* Исходящий generic-канал (отклик станции): УИМ его использует, НКУ-CAN и
 * демо — нет. task_sul_rx.c проверяет только NULL/не-NULL, без ветки по id. */
static void test_take_pending_tx_wired_only_for_uim(void)
{
    TEST_ASSERT_NOT_NULL(sul_registry_find(SUL_PROTOCOL_UIM)->take_pending_tx);
    TEST_ASSERT_NULL(sul_registry_find(SUL_PROTOCOL_NKU_CAN)->take_pending_tx);
    TEST_ASSERT_NULL(sul_registry_find(SUL_PROTOCOL_DEMO)->take_pending_tx);
}

/* УИМ: адрес индикатора — BYTE, но диапазон НЕ похож на НКУ-CAN (1..50 против
 * 0..15) — ещё одна проверка, что дескрипторный механизм не завязан на
 * конкретную форму параметра. */
static void test_uim_settings_descriptor_present(void)
{
    const sul_driver_t *p_drv = sul_registry_find(SUL_PROTOCOL_UIM);
    TEST_ASSERT_NOT_NULL(p_drv->p_settings);
    TEST_ASSERT_EQUAL_UINT8(1U, p_drv->p_settings->count);
    TEST_ASSERT_EQUAL(SUL_SETTINGS_BYTE, p_drv->p_settings->p_entries[0].type);
    TEST_ASSERT_EQUAL_UINT8(1U, p_drv->p_settings->p_entries[0].min);
    TEST_ASSERT_EQUAL_UINT8(50U, p_drv->p_settings->p_entries[0].max);

    /* Множество адресов НЕ непрерывно: 41..45 зарезервированы протоколом. */
    TEST_ASSERT_EQUAL_UINT8(41U, p_drv->p_settings->p_entries[0].gap_from);
    TEST_ASSERT_EQUAL_UINT8(45U, p_drv->p_settings->p_entries[0].gap_to);
}

/* gap_from == 0 — «разрыва нет»: у протоколов с непрерывным диапазоном поля
 * просто не указываются, designated-инициализация даёт 0. */
static void test_continuous_range_protocols_declare_no_gap(void)
{
    TEST_ASSERT_EQUAL_UINT8(
        0U, sul_registry_find(SUL_PROTOCOL_NKU_CAN)->p_settings->p_entries[0].gap_from);
    TEST_ASSERT_EQUAL_UINT8(
        0U, sul_registry_find(SUL_PROTOCOL_DEMO)->p_settings->p_entries[0].gap_from);
}
...
static void test_connection_timeout_set_per_protocol(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(3000U, sul_registry_find(SUL_PROTOCOL_UIM)->connection_timeout_ms);
}
...
static void test_each_driver_has_own_ctx(void)
{
    ...
    const sul_driver_t *p_uim  = sul_registry_find(SUL_PROTOCOL_UIM);

    TEST_ASSERT_NOT_NULL(p_nku->p_ctx);
    TEST_ASSERT_NOT_NULL(p_demo->p_ctx);
    TEST_ASSERT_NOT_NULL(p_uim->p_ctx);

    TEST_ASSERT_NOT_EQUAL(p_nku->p_ctx, p_demo->p_ctx);
    TEST_ASSERT_NOT_EQUAL(p_nku->p_ctx, p_uim->p_ctx);
    TEST_ASSERT_NOT_EQUAL(p_demo->p_ctx, p_uim->p_ctx);
}
...
int main(void)
{
    ...
    RUN_TEST(test_find_returns_uim);
    ...
    RUN_TEST(test_driver_ids_are_unique);
    RUN_TEST(test_nku_can_settings_descriptor_present);
    RUN_TEST(test_demo_settings_descriptor_present);
    RUN_TEST(test_uim_settings_descriptor_present);
    RUN_TEST(test_continuous_range_protocols_declare_no_gap);
    RUN_TEST(test_take_pending_write_wired_only_for_nku_can);
    RUN_TEST(test_take_pending_tx_wired_only_for_uim);
