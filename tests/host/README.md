...
### Добавление теста — правится ТОЛЬКО этот файл

`add_host_test()` — единственное место. В `CMakePresets.json` у `host-debug-build`/
`host-release-build` **намеренно нет списка `targets`**: собирается `all`, а `all` в
host-конфигурации — это ровно `tests/host` (плюс библиотеки, от которых тесты зависят).

Раньше список там был — 25 имён, продублированных в двух пресетах, — и за Фазы 3.7/3.8/3.10
отстал на три теста (`test_tft_app_heartbeat`, `test_bsp_reset`, `test_tft_app_boot_screen`).
CTest честно репортил их как `Not Run`, то есть штатная команда валидации молча не
проверяла новый код, а зелёный результат достигался только ручным
`cmake --build build/host-release`. Список удалён, чтобы у «добавить host-тест» не
осталось второй точки, которую можно забыть.
