...
| **3.7 watchdog: heartbeat-супервизор** ⬜ | **ОТЛОЖЕНО, делать перед Фазой 4 и до валидации УИМ на станции.** Кормление watchdog сейчас в единственном месте и в самой низкоприоритетной задаче — конструктивная хрупкость, уже выстрелившая на стенде. Прикрыто детектором голодания (даёт диагноз), но не защищено. См. раздел ниже. |
...
### 3.7 — Watchdog: heartbeat-супервизор (⬜ ОТЛОЖЕНО, делать перед Фазой 4)

> **Когда:** до старта Фазы 4 И до валидации протокола УИМ на живой станции.
> Обоснование срока: валидация УИМ — первый сценарий, где по шине пойдёт
> плотный трафик с обязательными откликами, т.е. нагрузка на `sul_rx_task`
> вырастет. Ловить голодание там же, где отлаживаем протокол, — плохая идея.

**Проблема.** `bsp_wdog_refresh()` вызывается из ЕДИНСТВЕННОГО места —
...
загрузчик, WDE — write-once), роняет плату. Это конструктивная хрупкость, а не
деталь реализации: она уже выстрелила на стенде (см.
[PERF_AND_HANG_INVESTIGATION.md](../../docs/PERF_AND_HANG_INVESTIGATION.md)).

**Что сейчас есть (сделано в разборе, НЕ является защитой).** Детектор
голодания: `sul_rx_task` отмечается `crash_log_feed_mark()`,
`crash_log_check_starvation()` из `input_poll_cb` (демон таймеров — его нельзя
выголодать) с порогом 4 c против 10 c у watchdog записывает причину и «хлебную
крошку» места залипания, затем сбрасывает плату сам. Это даёт ДИАГНОЗ и
управляемый сброс вместо молчаливого — но плата всё равно перезагружается.

**Что сделать.**
1. Каждая наблюдаемая задача (`sul_rx_task`, `render_task`, `menu_task`)
   публикует свой heartbeat — счётчик/тик последнего прохода цикла.
2. Кормит watchdog **демон программных таймеров** (высший приоритет в системе,
   выголодать нельзя) из `input_poll_cb` — но ТОЛЬКО если heartbeat'ы всех
   наблюдаемых задач свежие. Протухший heartbeat → кормление прекращается →
   watchdog срабатывает штатно, как и задумано.
3. Пороги свежести — на задачу свои: у `sul_rx_task` цикл 100 мс, у `menu_task`
   5 мс, `render_task` event-driven и может законно спать сколько угодно (для
   неё heartbeat — не «прошла итерацию», а «не залипла ВНУТРИ итерации»,
   т.е. проверять надо длительность текущей операции, а не факт прохода).
4. Детектор голодания и «хлебные крошки» — ОСТАВИТЬ: они дают причину, которую
   сам по себе watchdog не даёт.

**Что меняется в гарантии.** Сейчас watchdog косвенно означает «жива
задача-кормилец». После правки — «живы все наблюдаемые задачи», что и есть
искомое свойство. Формально это ослабление в одном месте (кормит контекст,
который почти невозможно застопорить) и усиление в трёх других. Требует явного
согласования — поэтому и вынесено в отдельный под-шаг, а не сделано походя.

**Осторожно при реализации.** `render_task` законно блокируется на
`ulTaskNotifyTake(portMAX_DELAY)` — наивный «heartbeat = прошла итерация»
уронит плату на простое. Нужна семантика «не залипла внутри операции»
(готовый механизм уже есть — `crash_log_set_breadcrumb()` ставится вокруг
долгих участков, можно завязать таймер на непустую крошку).

**Тесты.** Host-покрытие для чистой логики супервизора (набор heartbeat'ов +
пороги → кормить/не кормить) — отделима от FreeRTOS, как `mode_priority`.
На стенде: искусственная задержка в `render_task` дольше порога → сброс с
внятной причиной, а не молча.
