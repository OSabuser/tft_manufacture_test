...
| **3.9 уровни логов + уборка** ✅ | Тумблер «вкл/выкл» → три позиции (Выкл/Инфо/Отладка), компайл-тайм пол `LOG_LEVEL=4` для Release, каталог точек логирования, снятие `dummy_option` с бампом `SETTINGS_VERSION`. Подтверждено на стенде 2026-08-11 (см. раздел ниже). |
...
> **Статус: подтверждено на стенде 2026-08-11.** Host зелёный (набор `test_log`
> 28 → 33); ARM Debug/Release и все три образа в Release линкуются чисто.
>
> Проверено на живом устройстве: переключение Выкл/Инфо/Отладка даёт ожидаемый
> состав строк, уровень переживает перезагрузку, пункта «Общий?» в меню нет.
> Отдельно подтверждён фикс «тихой зоны» — при «Выкл» UART молчит полностью,
> включая строки crash/bringup (см. ниже).
...
**Проверка на стенде — пройдена (2026-08-11).** Все пункты: переключение
уровней и состав строк на каждом; сохранение уровня через перезагрузку;
отсутствие пункта «Общий?»; сброс настроек прежней версии в дефолты на первом
старте (ожидаемо, следствие `SETTINGS_VERSION` 2).

**🐛 Найдено на стенде и починено — «тихая зона» bring-up.** При выбранном
«Выкл» в UART всё равно уходили три строки: причина сброса и два сообщения
bring-up. Причина не в гейте, а в ПОРЯДКЕ: уровень логов живёт в настройках,
настройки — на QSPI, поэтому всё, что логировалось ДО подъёма QSPI, физически
нечем было отфильтровать — действовал дефолт «не резать ничего».

Порядок переставлен: молча поднять QSPI → загрузить настройки → применить
уровень → и только потом напечатать то же самое. Состав и порядок строк не
изменились, изменился момент печати.

Плата за это названа явно: `crash_log_report_previous()` уехал ПОСЛЕ
`bsp_qspi_init()`. При ОТКАЗЕ QSPI настройки берутся дефолтные, уровень —
«Инфо», и причина всё равно печатается. Потерять её можно только если
`bsp_qspi_init()` зависнет намертво — но такая плата не грузится в принципе, и
её диагностика это LED-паттерны загрузчика, а не наш UART.
...
### Решения, принятые в обсуждении перед фазой (2026-08-11)

Обсуждение шло до кода намеренно — формат бандла и схема layout дороги в
переделке. Здесь зафиксировано согласованное; открытые пункты названы явно.

#### Замеры — ГЕЙТ, делаются первыми

Три развилки решаются числами, а не рассуждением. Пока замеров нет — формат не
фиксируем:

| Замер | Что решает |
| --- | --- |
| Может ли PXP читать source прямо из XIP-окна FlexSPI | сырой ARGB против RLE |
| Скорость CPU-блита RLE и сырого ARGB (флеш / SDRAM кэшируемая / некэшируемая) | нужен ли кэш спрайтов вообще |
| Цена `kPXP_Rotate90` поверх нынешних 8–9 мс композита + ограничения (форматы, «output XOR process surface») | как поворачивать экран, см. ниже |
| Запись в AS транспонированно против последовательно | цена программного свопа осей |
| Время CRC региона на скорости чтения FlexSPI | подтверждает ленивую валидацию |

#### Формат — свойство ЗАПИСИ, а не всего бандла

Решающий довод — числами. Регион ассетов 11,7 МБ на минимальном W25Q128;
слайдшоу до 7 полноэкранных картинок:

| Панель | ARGB8888 | RGB565 |
| --- | --- | --- |
| TFT7 1024×600 | 2,34 МБ × 7 = **16,4 МБ** ✗ | 1,17 × 7 = **8,2 МБ** ✓ |
| TFT8 800×600 | 1,83 × 7 = **12,8 МБ** ✗ | 0,92 × 7 = **6,4 МБ** ✓ |
| TFT4 480×272 | 0,50 × 7 = 3,5 МБ ✓ | 1,75 МБ ✓ |

Сырой ARGB8888 для полноэкранного слайдшоу **физически не помещается** на
TFT7/TFT8. Но фону не нужен альфа-канал, а FB после §3.2.4 и так RGB565 — то
есть фон в 565 вдвое меньше И блитится прямой копией без конверсии.

Отсюда: **поле формата в TLV-записи**, а не один глобальный выбор.

| Что | Формат |
| --- | --- |
| полноэкранный фон | RGB565 без альфы |
| спрайт в области с альфой (`UserPic`) | ARGB8888 |
| плоская UI-графика (стрелки, пиктограммы) | RLE |
| глифы шрифта | RLE |

Поле стоит копейки заложить сейчас и дорого добавлять потом. Оптимизировать
фон под 565 сразу НЕ нужно: слайдшоу и реклама — редкий спецзаказ, типовой
стиль это сотни килобайт.

#### Шрифты в бандле — смещения вместо указателей

`tFont`→`tChar[]`→`tImage` держатся на указателях, которые проставляет линкер;
в бандле их быть не может. Формат шрифта в бандле: заголовок + отсортированная
таблица `{код, ширина, высота, смещение}` + RLE-блоб.

**Пиксельный цикл не меняется**: `draw_glyph()` уже принимает `const tImage*`,
и запись таблицы материализуется во временный `tImage` на стеке
(`data = база_бандла + смещение`). Меняется только поиск глифа.

Следствия: несколько шрифтов в стиле — обычные записи со своими id; разные
шрифты у клиентов — едут со стилем; резервные остаются вшитыми.

И: **«шрифт» и «спрайт на символ» структурно одно и то же** — таблица
`код → картинка`. Откуда картинки (растеризатор TTF или экспорт по одной) —
деталь паковщика, не формата. Выбирать сейчас не нужно.

#### Валидация — ленивая

CRC **индекса** на старте; CRC полезной нагрузки — при первом обращении к
ассету, с защёлкой результата. Считать CRC 11,7 МБ при загрузке — сотни мс при
нынешнем bring-up 120 мс. Ограничение ARCH §11 («рантайм не ревалидирует —
битое чтение даёт hardfault») этим сохраняется: проверка всё равно ДО
использования.

#### Кэш спрайтов — НЕ в объёме фазы

Свободно 24 МБ кэшируемой SDRAM и 3,2 МБ некэшируемой — место не ограничение.
Но: индикатор меняет этаж несколько раз в минуту, а не 60 раз в секунду;
плавность даёт двойная буферизация (§3.2.4), а не пропускная способность блита.

Кэш вводится **только по результатам замеров**, если они покажут нужду. Важная
связь: при CPU-блите кэш живёт в кэшируемой SDRAM, при PXP-блите обязан быть в
НЕкэшируемой (DMA-мастер) — то есть выбор блит-пути определяет и место кэша.

#### Стиль = ассеты + layout, атомарно

- **Сравниваем ГЕОМЕТРИЮ, а не модель панели.** Layout'у важны пиксели и
  ориентация, а не артикул: разные модели с одним разрешением должны принимать
  один стиль, а одна модель в двух ориентациях — это две разные геометрии.
  Модель панели — только текст в диагностике.
- **Валидация в ДВУХ моментах**: при записи (service_tui/апдейтер — отказать
  сразу) и **при загрузке**. Второе обязательно: плату могут перепровижинить
  после записи стиля (в сервисе заменили панель), и корректный стиль станет
  несовместимым. В рантайме — никогда (§11).
- **При несовпадении — резервный режим + явная диагностика.** Не масштабировать:
  layout, растянутый с 1024×600 на 480×272, даёт нечитаемую цифру, то есть
  функциональный отказ под видом работы. Диагностику писать уже есть куда —
  слот имени стиля на загрузочном экране (§3.10).
- **Рассинхрон регионов.** layout (`0x440000`) и ассеты (`0x452000`) физически
  разнесены и могут разъехаться. Заголовок layout несёт id/версию стиля и CRC
  ожидаемого бандла; не сошлось — откат.
- **Хранится ОДИН стиль.** Смена = перезапись региона (Фаза 7 это и так умеет).
  Каталог с переключением потребовал бы селектора в настройках и обработки
  «выбранный отсутствует» — сложность без сценария.
- **Ориентация — параметр ПРОВИЖИНИНГА**, не только свойство стиля: резервный
  режим тоже обязан рисоваться правильно на вертикально смонтированной панели,
  а стиля в этот момент нет. Сегодня в `settings_device_t` её нет вовсе —
  добавляется в Фазе 9, но заголовок стиля объявляет её с самого начала.

#### Поворот экрана — открыто, решается замером

Пины LR/UD (`SHLR`/`UPDN` на driver IC) дают **только зеркало**, транспонировать
оси не могут. 90° требует свопа осей, и кандидатов два:

| | A. Программный своп (как `OLD_PROJECT/modules/compositor`) | B. `kPXP_Rotate90` |
| --- | --- | --- |
| Проверено в бою | **да**, TFT7/TFT8 | нет |
| Цена PXP | нулевая | мерить |
| TFT4 (пинов нет) | зеркало тоже в софт — отдельная ветка | ничем не отличается |
| Доступ к памяти | **транспонированная запись** в НЕкэшируемую SDRAM: ни write-combining, ни burst | последовательный |

Вариант B привлекателен единообразием (TFT4 без исключений) и последовательной
записью, но не проверен. Решает замер. В обоих вариантах остаётся ограничение
эталона: `pitch` у PXP и ELCDIF — общий, физический.

**Архитектурно (независимо от выбора):** `gfx` и виджеты работают в
**ЛОГИЧЕСКОЙ** геометрии макета, поворот применяется ОДИН раз на композите.
Поворот на этапе паковки не годится — текст рендерится в рантайме из глифов, а
не готовым растром; пришлось бы генерировать повёрнутые шрифты.

#### Пайплайн стиля и инструмент

Две стадии, и это развязывает выбор инструмента:

1. **Источник → промежуточный каталог** (PNG + `manifest.json`), **коммитится**.
   Сегодня — экспорт из Figma; завтра, возможно, свой drag-drop редактор.
2. **Каталог → `style.bin`**, детерминированно, в CI.

Промежуточный формат означает, что **вопрос «Figma или свой редактор» можно не
решать сейчас** — стадия 2, схема layout, формат бандла и прошивка не меняются
ни на байт. Главный довод за свой редактор: проблема валидатора исчезает по
построению (нельзя выразить то, чего движок не нарисует). Довод против:
отдельный продукт со своим сопровождением. Решать — когда схема устоится.

Правила, которые фиксируются письменно (`docs/`) + шаблон-библиотека в Figma:
имена слоёв — контракт; **нераспознанное = ошибка сборки, а не игнор**
(молча потерянный виджет обнаружится на объекте); обязательный минимум ролей;
одна геометрия на стиль; приёмка — golden-render на хосте.

#### Заливка на стенде

`just host::flash-swd-style` по образцу `flash-swd-app-slot-debug`: pyocd,
`--base-address 0x452000`. Ноль нового инструментария; поле — Фаза 7.

#### Долг домена, вскрытый обсуждением

- **«Перевозка лежачих больных»** — режима нет в `sul_mode_t` вообще. Поле в
  `sul_result_t` + строка в таблице приоритетов; распознавание — НКУ-CAN и УКЛ
  (Фаза 8).
- **Настройки под ярус A**: грузоподъёмность и вместимость есть, но без
  редакторов; видимость логотипа и период слайдшоу — полей нет. Собрать **одним
  заходом** перед Фазой 5, чтобы `SETTINGS_VERSION` бампнуть один раз.
...
| Под-шаг | Содержание |
| --- | --- |
| **4.1 спрайты резервного режима** ⬜ | Стрелки и пиктограммы режимов — вкомпилированными в образ картинками вместо примитивов. Не требует бандла и делается ПЕРВЫМ: заставляет спроектировать общий блиттер, которым потом воспользуется 4.2. См. раздел ниже. |
| **4.2 TLV-бандл + паковщик + ридер** ⬜ | Формат, host-утилита, `services/assets`, заливка на стенд. |

### 4.1 — Спрайты резервного режима (⬜)

**Замысел.** Сейчас `ui/fallback` рисует стрелку примитивом (`gfx_draw_arrow()`),
а режимы — текстовой меткой. Заменить на настоящие картинки, вкомпилированные
в образ.

**Почему это НЕ нарушает контракт fallback.** ARCH §11 требует от `default`-
layout быть «asset-free и FS-free» — в смысле «не монтирует QSPI-FAT/SD, не
трогает TLV-ридер». Скомпилированная в образ картинка ничего из этого не
делает: она лежит в том же флеше, что и код, читается XIP, файловой системы не
требует. **Резервный режим уже содержит вшитые ассеты — это шрифты**
(`FloorFontFallback`, 38 глифов по 215pt). Спрайты — та же категория, не новая.
Формулировку в ARCH §11 и FALLBACK.md надо уточнить, иначе «asset-free» будут
читать как «никаких изображений вообще».

**Механизм уже есть, дописывать почти нечего:**
- `tImage` (`fonts.h`) — это ОБЫЧНАЯ картинка (`data`/`width`/`height`), а не
  шрифтовая структура; `tFont` лишь таблица `code → tImage`.
- `draw_glyph(const tImage*, x, y, color)` в `gfx.c` — уже готовый блиттер
  произвольной картинки, просто `static`.
- Тот же `lcd-image-converter`, которым сгенерированы шрифты, экспортирует и
  одиночные изображения в том же RLE/ARGB8888.

Работа сводится к: сгенерировать `.c`, объявить `extern const tImage`,
**опубликовать уже написанную функцию** как `gfx_draw_image()`.

**⚠️ Ограничение существующего пути — только альфа.** `draw_glyph()` берёт из
источника ТОЛЬКО альфу и красит её переданным цветом (тинтинг из §3.2.2).
Значит:

| Что рисуем | Готовность |
| --- | --- |
| одноцветные силуэты (стрелки, большинство пиктограмм) | работает как есть |
| многоцветные спрайты | нужен второй путь: тот же цикл RLE, но цвет из данных |

У одноцветного пути есть неочевидный плюс: **цвет задаётся в рантайме**. Один
спрайт стрелки годится для любой темы, пожарный режим красится красным без
второго файла.

**Бюджет — не ограничение** (замер по текущему образу):

| Шрифт | Глифов | Флеш | На глиф |
| --- | --- | --- | --- |
| `FloorFontFallback` (215pt) | 38 | 294 КБ | ~7.9 КБ |
| `JBMono24` | 143 | 101 КБ | 722 Б |
| `JBMono12` | 143 | 34 КБ | 246 Б |

`m_text` — 530 КБ из 2 МБ (25%). Крупная стрелка ≈ 8 КБ, десяток пиктограмм
режимов ≈ 80 КБ = +4% слота.

**Правило границы (важно зафиксировать до кода).** Спрайт в образе НЕЛЬЗЯ
обновить в поле — он едет с прошивкой, в отличие от бандла. Поэтому в образ
вшивается **только то, что обязано работать, когда бандл недоступен или битый**:
стрелки и пиктограммы режимов. Всё остальное живёт в бандле. Без явного правила
вшитых ассетов со временем станет много, и они начнут дублировать бандл.

**Общий блиттер — проектировать сразу.** 4.2 принесёт блит ассетов из бандла.
Если 4.1 опубликует свой `gfx_draw_image()`, а 4.2 напишет второй, независимый —
получатся две копии пиксельного цикла, которые разъедутся. Низкоуровневый вывод
должен быть один, различие форматов (RLE из образа против сырого ARGB из
бандла) — на входе.

**Тесты.** Host: `gfx` пиксельно не тестируется (как и `menu_view`), поэтому
проверяется выбор спрайта по режиму/направлению — чистая функция в `ui/fallback`
(режим → id спрайта), по образцу `mode_priority`. Стенд: все режимы и обе
стрелки на реальной панели.
...
### Роли виджетов и модель раскладки (согласовано перед Фазой 4)

Собрано с пользователем по реальным клиентским макетам. Набор конечен —
это каталог, а не открытое множество.

| Роль | Тип | Привязка | Обяз. | Без ассета |
| --- | --- | --- | --- | --- |
| `FloorNumber` | текст | `pos`, `lading_secs` | **да** | резервный шрифт (вшит) |
| `Direction` | спрайт | `direction` (нет/вверх/вниз/двойная) | **да** | резервные спрайты стрелок (§4.1) |
| `Mode` | спрайт | `mode` | **да** | резервные спрайты режимов (§4.1) |
| `ModeAux` | спрайт | `mode` — **тот же**, мелким значком в статус-строке | нет | ничего |
| `Dispatcher` | спрайт | `g_dispatcher_indication`: `CALLING`/`TALKING` | нет | ничего |
| `CabinCapacity` | контейнер | `max_load_kg`, `max_cap_persons` | нет | ничего |
| `CabinSerial` | спрайт + текст | `serial[10]` | нет | ничего |
| `Logo` | спрайт | настройка «показать/скрыть» | нет | ничего |
| `FullscreenUserPic` | спрайт | слайдшоу, до 7 картинок | нет | тёмный фон |
| `UserPic` | спрайт | слайдшоу в области с альфой | нет | ничего |
| `AdvText`, `AdvTextAux` | текст | текст из СТИЛЯ | нет | ничего |

Слайдшоу и реклама — **редкий спецзаказ**, не типовой стиль. Отсутствующая
роль обязана не стоить ничего: ни записи в layout, ни байта в бандле, ни ветки
в валидаторе.

#### `ModeAux` — домен НЕ трогается

Это не второй режим, а **второй виджет на те же данные** (мелкий значок в
статус-строке по требованию части клиентов). Таблица приоритетов и
`indication_task_t.mode` остаются как есть; расширять контроллер до «топ-2
режимов» не нужно.

#### `Dispatcher` — отдельная роль, рисуется ПОСЛЕДНЕЙ

Не сливается с `ModeAux`, и это как раз НЕ плодит сущности. Часть клиентов
требует, чтобы диспетчерский сигнал затирал режим и направление, часть — чтобы
показывался параллельно. Оба случая закрываются **размещением**: положили
поверх `Mode` — затирает, разнесли — параллельно.

Правило «рисуется последней» — жёсткое правило движка, а не соглашение
макета: диспетчерский вызов единственный работает при оборванной связи со
станцией (§3.4), и дать его случайно перекрыть нельзя.

Состояния (уточнено пользователем, совпадает с реализацией §3.4): пассажир
жмёт кнопку → станция подаёт `CALLING`; диспетчер ответил → станция СНИМАЕТ
`CALLING` и подаёт `TALKING`; разговор окончен → снимает и его. Отсюда опрос
**уровней**, а не фронтов; `TALKING` перебивает `CALLING`.

#### Контейнер — линейный стек со схлопыванием

`CabinCapacity` требует ровно этого: пара «иконка + отступ₁ + текст», две пары
подряд по горизонтали ИЛИ по вертикали с отступом₂, каждая пара скрывается
независимо (значение `0` = скрыто, конвенция уже есть в `settings_codec.c`).

```
Группа (абсолютный якорь из макета)
  направление: горизонтально | вертикально,  отступ: gap2
  ├── Пара: горизонтальный стек, отступ gap1 → иконка + текст «12500 кг»
  └── Пара: горизонтальный стек, отступ gap1 → иконка + текст «12 чел»
```

- **скрытый ребёнок ИСКЛЮЧАЕТСЯ из раскладки**, а не оставляет дыру (согласовано):
  одинокая пара во второй строке с пустой первой читается как сбой отрисовки;
- **единицы — часть текста** (`%u кг`), а не отдельный элемент: иначе в стеке
  появляется второй отступ и он перестаёт быть однородным;
- контейнер — общий кирпич, а не костыль под одну роль: загрузочный экран
  (§3.10) это ровно вертикальный стек строк.

**Границы, чтобы не выросло в вёрстку общего назначения.** Есть: направление,
один отступ, схлопывание, вложенность до 2–3 уровней. Нет: переносов,
процентов, выравнивания детей внутри контейнера, растяжения, отрицательных
отступов. Что не выражается — выражается абсолютным размещением.

#### ⚠️ Валидатор обязан проверять ХУДШИЙ случай, а не макет

**В макете дизайнер видит одно значение, а клиент вводит другое.** Нарисовано
«800 кг» — помещается; клиент выставил 12500 — пара стала шире на два символа
и уехала за край. Проявится **только у того клиента**, а на стенде и в макете
всё прекрасно.

Проверять надо максимум диапазона: грузоподъёмность 5 знаков, вместимость 2,
серийник 10, позиция 2 + знак. Проверка возможна ровно потому, что диапазоны
известны из `settings_t`.

Мелочь на будущее: `max_load_kg` — `uint16_t`, потолок 65535. Для лифта
достаточно; если понадобится больше — расширение поля и бамп версии настроек.

#### Текст: выравнивание нужно ВСЕГДА

Независимо от контейнеров: `1` и `12` в поле этажа обязаны не прыгать.
Выравнивание (left/right/center в своей рамке) — база; контейнер со
схлопыванием — надстройка поверх неё.
