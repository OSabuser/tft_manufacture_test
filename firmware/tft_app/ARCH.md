...
- **layout** — декларативная таблица виджетов (что рисуем: позиция/стрелка/режим/лого/…,
  размер, шрифт, привязка к полю модели). Формат — **TLV**, схема версионируется.

  > ⚠️ **Ревизия (обсуждение перед Фазой 4).** Изначально здесь было якорное
  > позиционирование с мотивировкой «один макет раскладывается под 480×272 и под 1024×600».
  > Принято обратное решение: **один стиль = одна геометрия** («фрейм = панель»).
  >
  > Причина: **якоря решают позицию, но не размер.** Цифра этажа, нарисованная под
  > 1024×600, физически не отрендерится на 480×272 — нужен другой ассет и другой кегль.
  > Раз ассеты всё равно пер-геометрия, пер-геометрийный layout стоит почти ничего сверху,
  > зато сохраняет WYSIWYG (дизайнер видит ровно то, что уедет на плату) и делает валидацию
  > стиля тривиальной: объявленная геометрия должна совпасть с провижинингом.
  >
  > Вместо якорей — абсолютное размещение плюс **линейные контейнеры** (стек с направлением
  > и отступом, скрытые дети схлопываются), см. PLAN.md Фаза 5.
...
не трогает TLV-ридер.

> ⚠️ **«asset-free» ≠ «без изображений».** Смысл термина — «не зависит от внешнего бандла»,
> а не «рисует только примитивами». Скомпилированная в образ картинка ничему из перечисленного
> не противоречит: она лежит в том же флеше, что и код, читается XIP, файловой системы не
> требует. Резервный режим вшитые ассеты **уже содержит** — это шрифты (`FloorFontFallback`,
> 38 глифов по 215pt, 294 КБ). Спрайты стрелок и режимов (PLAN.md §4.1) — та же категория.
>
> Граница: в образ вшивается **только то, что обязано работать, когда бандл недоступен или
> битый**. Спрайт в образе нельзя обновить в поле — он едет с прошивкой, поэтому список вшитого
> держится коротким осознанно, а не «пока не разрослось».

Зависимости — только `bsp_display` + `bsp_can`/`sul` + домен + вкомпилированные шрифты и спрайты
+ примитивы `gfx`. Рисует **этаж, стрелку направления и режим**, чёрный фон, без звука. Это же —
первый экран walking-skeleton (§PLAN, Фаза 1).
