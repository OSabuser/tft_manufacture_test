...
#define SETTINGS_VOLUME_LEVELS   5U /* индексы громкости: OFF/25/50/75/100     */
    ...
    typedef struct
    {
        ...
        uint8_t log_level;
        /* Длительность загрузочного экрана, секунды; 0 = не показывать (§3.10).
           Пункта меню НЕТ намеренно — уровень провижининга, рядового оператора
           не касается.

           Занял бывший `_pad`: байт для того и лежал, поэтому размер структуры
           и смещения `settings_user_t` НЕ изменились — `proto_slice` остался на
           месте. `SETTINGS_VERSION` всё равно бампнут, чтобы сохранённые
           страницы не притащили `_pad = 0` как «экран выключен». */
        uint8_t boot_screen_secs;
    } settings_device_t;
