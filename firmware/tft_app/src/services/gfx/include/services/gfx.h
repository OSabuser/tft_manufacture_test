...
#define SystemFontSmall JBMono12
    ...
    /**
 * @brief Как читать пиксели `tImage` (§4.1: формат — свойство КАРТИНКИ).
 *
 * Осознанно НЕ глобальный выбор проекта: RLE жмёт ПОВТОРЫ полного 32-битного
 * пикселя, поэтому силуэт и плоская многоцветная пиктограмма жмутся отлично
 * (замер по резервному шрифту — ×11.2, 294 КБ против 3303 КБ raw), а
 * фотографическое изображение с градиентами — почти никак. Для второго случая
 * RAW дешевле и по месту, и по коду. Ту же развилку Фаза 4.2 несёт полем в
 * TLV-записи бандла (см. PLAN «Формат — свойство ЗАПИСИ, а не всего бандла»).
 */
    typedef enum
    {
        GFX_IMAGE_RLE_ARGB8888 = 0, /**< поток RLE-блоков (lcd-image-converter / паковщик) */
        GFX_IMAGE_RAW_ARGB8888,     /**< сырые ARGB8888-пиксели подряд                     */
    } gfx_image_format_t;
 ...
 */
    ...
    /**
 * @brief Нарисовать картинку «как есть»: цвет И альфа берутся из пикселей
 *        источника (многоцветные спрайты).
 *
 * Тот же пиксельный цикл, что у глифов шрифта — различие форматов и цветовых
 * путей вынесено во вход, второй копии цикла в проекте нет (PLAN §4.1).
 * Рисует в AS, обрезается по экрану; полностью прозрачные RLE-прогоны
 * пропускаются целиком (ни чтения источника, ни записи в SDRAM).
 *
 * @param p_image  картинка; NULL/нулевые размеры — тихий no-op (по-виджетная
 *                 деградация, ARCH §11: отсутствующий ассет не роняет кадр)
 * @param format   как читать `p_image->data` (см. gfx_image_format_t)
 */
    void gfx_draw_image(const tImage *p_image, gfx_image_format_t format, uint16_t x, uint16_t y);

    /**
 * @brief Нарисовать картинку ТИНТОМ: из источника берётся только альфа, цвет —
 *        параметр @p color (одноцветные силуэты, тот же путь, что у шрифтов).
 *
 * Смысл именно в рантайм-цвете: один спрайт стрелки годится для любой темы, а
 * пиктограмма пожарного режима красится красным без второго файла в образе.
 */
    void gfx_draw_image_tinted(const tImage *p_image, gfx_image_format_t format, uint16_t x,
                               uint16_t y, gfx_color_t color);
    ...
    /** Простая треугольная стрелка — примитив. Резерв резерва: рисуется, когда
     *  спрайта стрелки в образе нет (§4.1 — деградация по-виджетно). */
