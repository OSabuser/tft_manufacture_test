...
- **Спецрежим** — короткая текстовая метка системным шрифтом (`SystemFont`). Это **safe-mode**
  ...
  > **Метка ВМЕСТО этажа — упрощение именно fallback'а, не правило домена.** Домен разрешает
  > режиму сосуществовать с позицией: в Фазах 4/5 layout выводит спрайт режима ОДНОВРЕМЕННО с
  > номером этажа и стрелкой (см. [MODE_PRIORITY.md §2.1](MODE_PRIORITY.md)). Здесь этаж
  > замещается потому, что asset-free рендер располагает одной текстовой строкой и обязан
  > оставаться минимальным — а не потому, что режим «отменяет» позицию.
- **Диспетчерский вход** (§3.4) — та же safe-mode текстовая метка (переиспользует слот режимов:
  `MODE_Y`/`SystemFont`), но проверяется ПЕРВЫМ, до `mode`. Слова временные, «ВЫЗОВ»/«ОТВЕТ» (CALLING/TALKING).
