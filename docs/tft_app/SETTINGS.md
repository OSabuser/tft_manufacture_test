...
| `device.boot_screen_secs` | `uint8_t` | provisioning | `3` c; `0` = не показывать | **нет намеренно** — уровень провижининга, рядового оператора не касается (§3.10). Занял бывший `_pad`, поэтому смещения `user.*` не сдвинулись |
...
> **`SETTINGS_VERSION` = 3.** Бампалась дважды подряд: `2` — снятие
> UX-эксперимента `dummy_option` (§3.9, поле удалено → `proto_slice` сдвинулся),
> `3` — `_pad` стал `boot_screen_secs` (§3.10; размер и смещения те же, но
> старая страница дала бы `0` = «экран выключен» вместо дефолта). Обе — по
> одному правилу: **если байт поменял смысл или место, старые страницы должны
> уйти в дефолты, а не быть разобраны по новой раскладке.** С версией ОБРАЗА
> (`project(tft_app VERSION)`) это никак не связано — см.
> [RELEASE_PROCESS.md §1.1](../RELEASE_PROCESS.md).
